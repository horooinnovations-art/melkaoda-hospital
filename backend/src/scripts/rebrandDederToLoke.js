/**
 * Rebrand Deder → Loke across text fields in the shared MySQL database.
 * Content stays the same; only hospital name / "Deder" wording changes.
 *
 * Usage (from backend/, with DB_* env pointing at the live MySQL):
 *   npm run rebrand:deder-to-loke
 */
import dotenv from 'dotenv';
dotenv.config();

import pool from '../config/db.js';
import { rebrandText } from '../utils/settings.js';
...
async function rebrandTable({ table, columns, where }) {
  ...
  for (const row of rows) {
    ...
    for (const column of existingColumns) {
      ...
      if (typeof original !== 'string' || !/deder/i.test(original)) continue;
      const next = rebrandText(original);
...
async function main() {
  console.log('Rebranding Deder → Loke in database content...');
  let total = 0;
  for (const entry of TABLE_COLUMNS) {
    total += await rebrandTable(entry);
  }

  // Explicit org identity keys
  await pool.query(
    `UPDATE settings SET value = 'Loke General Hospital', updated_at = NOW()
     WHERE \`key\` IN ('organization_name', 'site_name', 'name')`
  );
  await pool.query(
    `UPDATE settings SET value = REPLACE(REPLACE(value, 'Deder General Hospital', 'Loke General Hospital'), 'Deder', 'Loke'), updated_at = NOW()
     WHERE value LIKE '%Deder%' OR value LIKE '%deder%'`
  );

  console.log(`\nDone. ${total} content row(s) rebranded.`);
  process.exit(0);
}
